...
Installate [Docker Desktop](https://www.docker.com/get-started) sul computer. Per testare un lavoratore personalizzato, eseguire il comando seguente nella directory principale dell&#39;applicazione:
...
Viene eseguito un framework di test dell&#39;unità personalizzato per  azioni dell&#39;applicazione di Asset compute nel progetto, come descritto di seguito. Viene collegato tramite una configurazione nel file `package.json`. È inoltre possibile disporre di unit test JavaScript come Jest. `aio app test` esegue entrambi.
...
L&#39;output dettagliato del test, compresi i registri dell&#39;applicazione personalizzata, sono disponibili nella cartella `build` alla radice dell&#39;app Firefly, come dimostrato nell&#39;output `aio app test`.

### Servizi esterni {#mock-external-services}

È possibile simulare le chiamate di servizio esterne nelle azioni definendo i file `mock-<HOST_NAME>.json` nei casi di test, dove HOST_NAME è l&#39;host che si desidera controllare. Un esempio di utilizzo è un&#39;applicazione che esegue una chiamata separata a S3. La nuova struttura del test sarà simile alla seguente:
...
Il file del mock è una risposta HTTP in formato JSON. Per ulteriori informazioni, consultare [questa documentazione](https://www.mock-server.com/mock_server/creating_expectations.html). Se vi sono più nomi host da simulare, definite più file `mock-<mocked-host>.json`. Di seguito è riportato un esempio di file di modello per `google.com` denominato `mock-google.com.json`:
...
L&#39;esempio `worker-animal-pictures` contiene un file [mock](https://github.com/adobe/asset-compute-example-workers/blob/master/projects/worker-animal-pictures/test/asset-compute/worker-animal-pictures/simple-test/mock-upload.wikimedia.org.json) per il servizio Wikimedia con cui interagisce.

#### Condivisione di file tra i test case {#share-files-across-test-cases}

È consigliabile utilizzare i simboli relativi se si condividono script `file.*`, `params.json` o `validate` tra più test. Sono supportati con git. Accertatevi di assegnare ai file condivisi un nome univoco, in quanto potrebbero essere presenti file diversi. Nell&#39;esempio seguente, i test combinano e corrispondono ad alcuni file condivisi, e i relativi:
...
I casi di test di errore non devono contenere un file previsto `rendition.*` e devono definire la `errorReason` prevista all&#39;interno del file `params.json`.
...
Vedere l&#39;elenco completo e la descrizione di [ motivi di errore Asset compute](https://github.com/adobe/asset-compute-commons#error-reasons).
...
Molti di questi passaggi sono generalmente automatizzati da `aio` fuori dalla casella, vedere la sezione Debug dell&#39;applicazione nella [Documentazione Firefox](https://www.adobe.io/apis/experienceplatform/project-firefly/docs.html#!AdobeDocs/project-firefly/master/getting_started/first_app.md). Per il momento, i passaggi seguenti includono una soluzione alternativa.

1. Installate la versione più recente di [wskdebug](https://github.com/apache/openwhisk-wskdebug) da GitHub e dal [ngrok](https://www.npmjs.com/package/ngrok) opzionale.
...
1. Chiudete tutte le istanze delle app aperte tramite `aio app run`.
1. Distribuite il codice più recente utilizzando `aio app deploy`.
1. Eseguire solo il Devtool del Asset compute  utilizzando `npx adobe-asset-compute devtool`. Lo tenga aperto.
1. Nell&#39;editor del codice VS, aggiungi la seguente configurazione di debug al `launch.json`:
   ...
   Recupera il NOME AZIONE dall&#39;output di `aio app deploy`. È simile a `Your deployed actions -> TypicalCoffeeCat-0.0.1/__secured_worker`.

1. Selezionare `wskdebug worker` dalla configurazione di esecuzione/debug e premere l&#39;icona di riproduzione. Attendere che inizi finché non viene visualizzata la finestra **[!UICONTROL Pronto per le attivazioni]** nella **[!UICONTROL Console di debug]**.

1. Fare clic su **[!UICONTROL run]** nel Devtool. Potete visualizzare le azioni eseguite nell&#39;editor del codice VS e i registri iniziano a essere visualizzati.
